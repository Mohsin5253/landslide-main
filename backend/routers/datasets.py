...
def preview_dataset(filename: str):
            ...
            for i, line in enumerate(slice_lines[:290]):
                ...
                if len(parts) >= 2:
                    ...
                    pairs.append({
                        ...
                        "status": "SEGMENTED",
                        "image_url": f"{BACKEND_URL}/uploads/{img_name}" if os.path.exists(os.path.join(upload_dir, img_name)) else f"{BACKEND_URL}/flood-images/{img_name}",
                        "mask_url": f"{BACKEND_URL}/uploads/{mask_name}" if os.path.exists(os.path.join(upload_dir, mask_name)) else f"{BACKEND_URL}/masks/{mask_name}"
                    })
            return {
                "type": "mask_manifest",
                "filename": filename,
    ...
    mask_url = None
    if has_mask:
        mask_url = f"{BACKEND_URL}/uploads/{matching_mask}" if os.path.exists(os.path.join(upload_dir, matching_mask)) else f"{BACKEND_URL}/masks/{matching_mask}"
    elif is_mask:
        mask_url = f"{BACKEND_URL}/uploads/{filename}"

    image_url = None
    if has_img:
        image_url = f"{BACKEND_URL}/uploads/{matching_img}" if os.path.exists(os.path.join(upload_dir, matching_img)) else f"{BACKEND_URL}/flood-images/{matching_img}"
    else:
        image_url = f"{BACKEND_URL}/uploads/{filename}" if os.path.exists(os.path.join(upload_dir, filename)) else f"{BACKEND_URL}/flood-images/{filename}"
