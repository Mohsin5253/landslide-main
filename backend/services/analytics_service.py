...
class AnalyticsService:
    ...
    def _analyze_image_file(self, filepath: str, filename: str, idx: int) -> Dict:
        ...
        # URLs
        image_url = f"{BACKEND_URL}/uploads/{filename}" if os.path.exists(os.path.join(UPLOADS_DIR, filename)) else f"{BACKEND_URL}/flood-images/{filename}"
        base_name, _ = os.path.splitext(filename)
        mask_name = f"{base_name}.png"
        mask_url = f"{BACKEND_URL}/uploads/{mask_name}" if os.path.exists(os.path.join(UPLOADS_DIR, mask_name)) else f"{BACKEND_URL}/masks/{mask_name}"
