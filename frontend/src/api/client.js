...
const WS_ORIGIN = BACKEND_ORIGIN.replace(/^https?/, (m) => m === 'https' ? 'wss' : 'ws');
const BASE_URL = API_BASE;

// Export BACKEND_ORIGIN for use in other modules
...
export const createSensorWS = () => {
  const ws = new WebSocket(`${WS_ORIGIN}/api/sensors/ws/live`);
  return ws;
};
...
export const createAlertsWS = () => {
  ...
  return new WebSocket(`${WS_ORIGIN}/api/alerts/ws`);
};
...
export const downloadReport = (reportId, format) => {
  const token = localStorage.getItem('access_token');
  window.open(`${BACKEND_ORIGIN}/api/report/${reportId}/download/${format}?token=${token}`, '_blank');
};
