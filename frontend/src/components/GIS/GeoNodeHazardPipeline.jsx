...
const DEFAULT_GEONODE_LAYERS = [
  {
    ...
    download_url: BACKEND_ORIGIN + '/api/hazards/live_global_hazards.geojson',
    wfs_url: BACKEND_ORIGIN + '/api/hazards/live',
    wms_url: BACKEND_ORIGIN + '/api/hazards/geonode/wms?request=GetCapabilities',
  ...
  },
  {
    ...
    format: 'GeoJSON / OGC WFS 2.0',
    url: '/api/hazards/ner',
    download_url: BACKEND_ORIGIN + '/api/hazards/ner_corridors.geojson',
    wfs_url: BACKEND_ORIGIN + '/api/hazards/ner',
    wms_url: BACKEND_ORIGIN + '/api/hazards/geonode/wms?layers=geonode:ner_landslide_corridors',
    update_interval_sec: 900,
    schema_attributes: [
  ...
  },
  {
    ...
    download_url: BACKEND_ORIGIN + '/api/hazards/earthquakes.geojson',
    wfs_url: BACKEND_ORIGIN + '/api/hazards/earthquakes',
    wms_url: BACKEND_ORIGIN + '/api/hazards/geonode/wms?layers=geonode:earthquake_vectors',
    update_interval_sec: 900,
    schema_attributes: [
  ...
  },
  {
    ...
    download_url: BACKEND_ORIGIN + '/api/hazards/cyclones.geojson',
    wfs_url: BACKEND_ORIGIN + '/api/hazards/cyclones',
    wms_url: BACKEND_ORIGIN + '/api/hazards/geonode/wms?layers=geonode:cyclone_tracks',
    update_interval_sec: 900,
    schema_attributes: [
  ...
  },
  {
    ...
    download_url: BACKEND_ORIGIN + '/api/hazards/volcanoes.geojson',
    wfs_url: BACKEND_ORIGIN + '/api/hazards/volcanoes',
    wms_url: BACKEND_ORIGIN + '/api/hazards/geonode/wms?layers=geonode:volcanic_alerts',
    update_interval_sec: 900,
    schema_attributes: [
  ...
  },
  {
    ...
    download_url: BACKEND_ORIGIN + '/api/hazards/landslides.geojson',
    wfs_url: BACKEND_ORIGIN + '/api/hazards/landslides',
    wms_url: BACKEND_ORIGIN + '/api/hazards/geonode/wms?layers=geonode:landslide_scarps',
    update_interval_sec: 900,
    schema_attributes: [
                ...
                <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 460, overflowY: 'auto', paddingRight: 4 }}>
                  ...
                  <div style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(168, 85, 247, 0.25)', borderRadius: 8, padding: 12 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
                      <a
                        href={currentLayer.download_url?.startsWith('http') ? currentLayer.download_url : `${BACKEND_ORIGIN}${currentLayer.download_url}`}
                        download
                        style={{
                          background: 'rgba(0, 229, 255, 0.15)',
                          color: CYAN,
                      ...
                      <button
                        onClick={() => copyToClipboard(currentLayer.wfs_url || `${BACKEND_ORIGIN}${currentLayer.url}`, `wfs_${currentLayer.name}`)}
                        style={{
                      ...
                      <button
                        onClick={() => copyToClipboard(currentLayer.wms_url || BACKEND_ORIGIN + '/api/hazards/geonode/wms', `wms_${currentLayer.name}`)}
                        style={{
...
{'import geopandas as gpd\ngdf = gpd.read_file("' + BACKEND_ORIGIN + '/api/hazards/live_global_hazards.geojson")\nprint(f"Loaded {len(gdf)} active hazard vectors into GeoDataFrame")'}
                    </pre>
                  </div>
                </div>
              )}
